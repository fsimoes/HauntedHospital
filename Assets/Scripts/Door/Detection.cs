...
public class Detection : MonoBehaviour
{
  ...
  //  public GameObject CrosshairPrefab;
    ...
    void Update()
    {
        ...
        //mouseRay.direction = new Vector3(mouseRay.direction.x, 0.5f , mouseRay.direction.y);
        //Ray ray = new Ray(transform.position, mouseRay.direction); //Camera.main.ViewportPointToRay(new Vector3(0.5F, 0.0F, 5F));
        ...
        {
            if (hit.collider.tag == "Door" && Vector3.Distance(hit.transform.position,transform.position) < Reach)
            {
                InReach = true;
